...
FILTER_MODELS_DIR = f"models/filter_res34_5_v{VERSION}"
HYPERPARAM_FILE = "data/segmentation/SAM2hyperparameters_v1.4.json"
...
TRAIN_DIR = f"data/versions/{VERSION}/train" 
...
MASK_DATA_PATH = f"data/segmentation/maskloader_128_tolerance=0.2_v{VERSION}.pt"
...
CLASSES_FILE = "data/classes_v21.json"
...
UPSAMPLE = 1000
#Number of classes to include in each of the top-k accuracy calculations during filter and ensembler training validation
TOP_KS = (3, 5)

#Run the trained model on the images in TRAIN_DIR
#and obtain evaluation metrics for data
EVAL = False
VAL_SIZE = 0.05  #Proportion of images we want in the held-out set
#Optionally, save the side-by-side comparisons
SAVE_IMG = True
FIG_SIZE = (16, 9)
#Data from yellowfin to be merged with prediction metric data
METADATA = "data/metadata/SR_01_SR_04_Photo_MetaData.xlsx"

# Test-time augmentation for coral filter training validation and inference - number of augmented data views
FILTER_TTA = 4
# Optionally fix the random views for reproducible TTA runs (set to an int)
FILTER_TTA_SEED = 42
...
#ResNet34, 5 models, upsampling=1000 + bootstrapping
#Ensemble model trained with out-of-sample accuracy: 0.9147, Recall: 0.8971, Precision: 0.8458
#Ensemble model trained with out-of-sample accuracy: 0.9213, Recall: 0.9191, Precision: 0.8757
#                                          accuracy: 0.9231, Recall: 0.8713, Precision: 0.9011