#!/usr/bin/env bash
# compile.sh -- Build responses.pdf from responses.tex
#
# No bibliography needed (responses.tex is self-contained), so only two
# pdflatex passes are run to settle cross-references.
#
# Usage: ./compile.sh
...
echo "==> [3/3] pdflatex (re-running until cross-references settle) ..."
pass=2
while (( pass <= MAX_RERUNS + 1 )); do
  log="$LOG_DIR/pdflatex_${pass}.log"
  run_pdflatex "$log"
  if grep -qi "Rerun to get \|there were undefined references" "$log"; then
    echo "    pass $pass: still settling, rerunning ..."
    pass=$(( pass + 1 ))
    continue
  fi
  break
...
echo ""
echo "==> Warnings from the final pass:"
